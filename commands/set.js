...
// and owners only. Used for changing prefixes and role names and such. Note
// that there's no 'checks' in this basic version - no config 'types' like Role,
// String, Int, etc... It's basic, to be extended with your deft hands! Note
// the **destructuring** here. instead of `args` we have : [action, key,
// ...value] This gives us the equivalent of either: const action = args[0];
// const key = args[1]; const value = args.slice(2); OR the same as: const
// [action, key, ...value] = args;
exports.run = async (client, message, [
  action, key, ...value
], _level) => { // eslint-disable-line no-unused-vars
  ...
  if (action === 'edit') {
    ...
    // setProp is an enmap feature, it defines a single property of an object in an
    // enmap key/value pair.
    client
      .settings
      .setProp(message.guild.id, key, value.join(' '));
  ...
  if (action === 'reset') {
    ...
    // Good demonstration of the custom awaitReply method in
    ...
    if (['y', 'yes'].includes(response.toLowerCase())) {
      ...
      client
        .settings
        .set(message.guild.id, overrides);
      message.reply(`${key} was successfully reset.`);
    } else
    // If they respond with n or no, we inform them that the action has been
    // cancelled.
    ...
    message
      .channel
      .send(inspect(settings), {code: 'json'});
  }
};
