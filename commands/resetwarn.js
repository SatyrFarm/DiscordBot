exports.run = async (client, message, args) => { // eslint-disable-line no-unused-vars
  ...
  //If the user does not define what number to reset the user warns to, reset the warns to 0
  if (!warnsToAdd) {
    client.warns.delete(key);
    message.channel.send(`${user.tag} has been reset to 0 warns.`);
  } else {
    client.warns.setProp(key, "warns", warnsToAdd);
    const userwarns = parseInt(client.warns.getProp(key, "warns"), 10);
    message.channel.send(`${user.tag} has been set to ${userwarns} warns.`);
  }
