...
exports.run = async (client, message, _args, level) => {
  ...
  if (!categoryId) {
    return message.reply('Please set the id of the closed tickets category. To set use `+set edit closedTi' +
        'cketsId <categoryId>`');
  }
    ...
    if (openTickets.indexOf(channelId) > -1 || isMod) {
      ...
      message
        ...
        .then((u) => {
          ...
          client
            .tickets
            .setProp(key, 'openTickets', openTickets);
          client
            .tickets
            .setProp(key, 'closedTickets', [
              ...closedTickets,
              channelId,
            ]);
      ...
      return message.reply('Either this channel is not a ticket, or this channel is not your ticket. In both' +
          ' cases you are unable to resolve this ticket.');
    }
  } else {
    return message.reply('You do not have any open tickets!');
  }
