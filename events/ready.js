module.exports = async client => {
  ...
  client.logger.log(`I am ready as ${client.user.tag}, ready to serve ${client.users.size} users in ${client.guilds.size} servers.`, "ready");
  ...
  client.guilds.forEach(guild => {
    ...
    if (guild.me.hasPermission('MANAGE_CHANNELS') && !logs) {
      ...
      client.logger.info(`Created the logging chat on the server ${guild.name}.`)
    }
    if (!guild.me.hasPermission('MANAGE_CHANNELS') && !logs) {
      client.logger.error(`The logs channel does not exist and tried to create the channel but I am lacking permissions for the server ${guild.name}.`);
    }
  });
